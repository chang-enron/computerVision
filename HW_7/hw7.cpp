...
int main(int argc,char** argv )
{
    ...
    for(i=0; i<64; i++)//interior 1:white 2:white && 4connected
    {
        for(j=0; j<64; j++)
        {
            if(symbolic.at<unsigned char>(i,j) == 255)
            {
                if(symbolic.at<unsigned char>(i+1, j) == 255 && symbolic.at<unsigned char>(i, j+1) == 255 && symbolic.at<unsigned char>(i-1, j) == 255  && symbolic.at<unsigned char>(i, j-1) ==255 && symbolic.at<unsigned char>(i-1, j-1) == 255 && symbolic.at<unsigned char>(i-1, j+1) == 255 &&    symbolic.at<unsigned char>(i+1, j-1) == 255 && symbolic.at<unsigned char>(i-1, j-1) == 255)
                    interior[i][j] = 2;
                else
                    interior[i][j] = 1;
            }   
